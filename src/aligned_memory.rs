...
impl<const ALIGN: usize> AlignedMemory<ALIGN> {
    ...
    pub fn fill_write(&mut self, num: usize, value: u8) -> std::io::Result<()> {
        ...
        if self.zero_up_to_max_len && value == 0 {
            // Invariant: If zero_up_to_max_len is true, all memory up to max_len must be zeroed
            #[cfg(debug_assertions)]
            {
                // Verify that the memory we're about to "allocate" is actually zeroed
                // SAFETY: We're creating a slice from uninitialized memory that should be zeroed.
                // This is only for verification in debug mode and doesn't affect release builds.
                let verify_slice = unsafe {
                    std::slice::from_raw_parts(
                        self.mem.as_ptr().add(self.mem.len()),
                        new_len.saturating_sub(self.mem.len()),
                    )
                };
                debug_assert!(
                    verify_slice.iter().all(|&b| b == 0),
                    "zero_up_to_max_len is true but memory is not zeroed!"
                );
            }
            // SAFETY: Everything up to `max_len` is zeroed and no shrinking is allowed.
            // The invariant zero_up_to_max_len guarantees all memory is pre-zeroed.
            unsafe {
    ...
    ///
    /// # Pre-conditions
    /// - There must be enough capacity: mem.len() + size_of::<T>() <= align_offset + max_len
    ///
    /// # Panics
    /// - In debug builds, panics if pre-conditions are violated
    /// - In release builds, aborts the process if pre-conditions are violated
    pub unsafe fn write_unchecked<T: Pod>(&mut self, value: T) {
        ...
        {
            // In release mode, abort if invariant is violated to prevent memory corruption
            if new_len > self.align_offset.saturating_add(self.max_len) {
                eprintln!(
                    "FATAL: write_unchecked invariant violated: new_len={}, max={}",
                    new_len,
                    self.align_offset.saturating_add(self.max_len)
                );
                std::process::abort();
            }
        }
    ...
    ///
    /// # Pre-conditions
    /// - There must be enough capacity: mem.len() + value.len() <= align_offset + max_len
    ///
    /// # Panics
    /// - In debug builds, panics if pre-conditions are violated
    /// - In release builds, aborts the process if pre-conditions are violated
    pub unsafe fn write_all_unchecked(&mut self, value: &[u8]) {
        ...
        #[cfg(not(debug_assertions))]
        {
            ...
            if new_len > self.align_offset.saturating_add(self.max_len) {
                eprintln!(
                    "FATAL: write_all_unchecked invariant violated: new_len={}, max={}",
                    new_len,
                    self.align_offset.saturating_add(self.max_len)
                );
                std::process::abort();
