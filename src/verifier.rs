...
use crate::{
    ebpf,
    program::{BuiltinFunction, FunctionRegistry, TBPFVersion},
...
fn check_registers(
    ...
    match (insn.dst, store) {
        (0..=9, _) | (10, true) => Ok(()),
        (10, false) if tbpf_version.dynamic_stack_frames() && insn.opc == ebpf::ADD64_IMM => Ok(()),
...
fn check_callx_register(
    ...
    let reg = if tbpf_version.callx_uses_src_reg() {
        insn.src as i64
    } else {
        insn.imm
    };
...
/// Mandatory verifier for tos programs to run on-chain
#[derive(Debug)]
pub struct RequisiteVerifier {}
impl Verifier for RequisiteVerifier {
    /// Check the program against the verifier's rules
    #[rustfmt::skip]
    fn verify<C: ContextObject>(prog: &[u8], _config: &Config, tbpf_version: TBPFVersion, _function_registry: &FunctionRegistry<usize>, syscall_registry: &FunctionRegistry<BuiltinFunction<C>>) -> Result<(), VerifierError> {
        check_prog_len(prog)?;

        let program_range = 0..prog.len() / ebpf::INSN_SIZE;
        let mut function_range = program_range.start..program_range.end;
        let mut insn_ptr: usize = 0;
        if tbpf_version.enable_stricter_verification() && !ebpf::get_insn(prog, insn_ptr).is_function_start_marker() {
        ...
        while (insn_ptr + 1) * ebpf::INSN_SIZE <= prog.len() {
            ...
            if tbpf_version.enable_stricter_verification() && insn.is_function_start_marker() {
            ...
            match insn.opc {
                ebpf::LD_DW_IMM if !tbpf_version.disable_lddw() => {
                    check_load_dw(prog, insn_ptr)?;
                    insn_ptr += 1;
                },

                // BPF_LDX class
                ebpf::LD_B_REG  if !tbpf_version.move_memory_instruction_classes() => {},
                ebpf::LD_H_REG  if !tbpf_version.move_memory_instruction_classes() => {},
                ebpf::LD_W_REG  if !tbpf_version.move_memory_instruction_classes() => {},
                ebpf::LD_DW_REG if !tbpf_version.move_memory_instruction_classes() => {},

                // BPF_ST class
                ebpf::ST_B_IMM  if !tbpf_version.move_memory_instruction_classes() => store = true,
                ebpf::ST_H_IMM  if !tbpf_version.move_memory_instruction_classes() => store = true,
                ebpf::ST_W_IMM  if !tbpf_version.move_memory_instruction_classes() => store = true,
                ebpf::ST_DW_IMM if !tbpf_version.move_memory_instruction_classes() => store = true,

                // BPF_STX class
                ebpf::ST_B_REG  if !tbpf_version.move_memory_instruction_classes() => store = true,
                ebpf::ST_H_REG  if !tbpf_version.move_memory_instruction_classes() => store = true,
                ebpf::ST_W_REG  if !tbpf_version.move_memory_instruction_classes() => store = true,
                ebpf::ST_DW_REG if !tbpf_version.move_memory_instruction_classes() => store = true,
                ...
                ebpf::SUB32_IMM  => {},
                ebpf::SUB32_REG  => {},
                ebpf::MUL32_IMM  if !tbpf_version.enable_pqr() => {},
                ebpf::MUL32_REG  if !tbpf_version.enable_pqr() => {},
                ebpf::LD_1B_REG  if tbpf_version.move_memory_instruction_classes() => {},
                ebpf::DIV32_IMM  if !tbpf_version.enable_pqr() => { check_imm_nonzero(&insn, insn_ptr)?; },
                ebpf::DIV32_REG  if !tbpf_version.enable_pqr() => {},
                ebpf::LD_2B_REG  if tbpf_version.move_memory_instruction_classes() => {},
                ebpf::OR32_IMM   => {},
                ebpf::OR32_REG   => {},
                ebpf::AND32_IMM  => {},
                ebpf::AND32_REG  => {},
                ...
                ebpf::RSH32_REG  => {},
                ebpf::NEG32      if !tbpf_version.disable_neg() => {},
                ebpf::LD_4B_REG  if tbpf_version.move_memory_instruction_classes() => {},
                ebpf::MOD32_IMM  if !tbpf_version.enable_pqr() => { check_imm_nonzero(&insn, insn_ptr)?; },
                ebpf::MOD32_REG  if !tbpf_version.enable_pqr() => {},
                ebpf::LD_8B_REG  if tbpf_version.move_memory_instruction_classes() => {},
                ebpf::XOR32_IMM  => {},
                ebpf::XOR32_REG  => {},
                ebpf::MOV32_IMM  => {},
                ebpf::MOV32_REG  => {},
                ...
                ebpf::ARSH32_REG => {},
                ebpf::LE         if !tbpf_version.disable_le() => { check_imm_endian(&insn, insn_ptr)?; },
                ebpf::BE         => { check_imm_endian(&insn, insn_ptr)?; },

                // BPF_ALU64_STORE class
                ebpf::ADD64_IMM  if insn.dst == ebpf::FRAME_PTR_REG as u8 && tbpf_version.dynamic_stack_frames() => {
                ...
                ebpf::SUB64_IMM  => {},
                ebpf::SUB64_REG  => {},
                ebpf::MUL64_IMM  if !tbpf_version.enable_pqr() => {},
                ebpf::ST_1B_IMM  if tbpf_version.move_memory_instruction_classes() => store = true,
                ebpf::MUL64_REG  if !tbpf_version.enable_pqr() => {},
                ebpf::ST_1B_REG  if tbpf_version.move_memory_instruction_classes() => store = true,
                ebpf::DIV64_IMM  if !tbpf_version.enable_pqr() => { check_imm_nonzero(&insn, insn_ptr)?; },
                ebpf::ST_2B_IMM  if tbpf_version.move_memory_instruction_classes() => store = true,
                ebpf::DIV64_REG  if !tbpf_version.enable_pqr() => {},
                ebpf::ST_2B_REG  if tbpf_version.move_memory_instruction_classes() => store = true,
                ebpf::OR64_IMM   => {},
                ebpf::OR64_REG   => {},
                ebpf::AND64_IMM  => {},
                ebpf::AND64_REG  => {},
                ...
                ebpf::RSH64_REG  => {},
                ebpf::ST_4B_IMM  if tbpf_version.move_memory_instruction_classes() => store = true,
                ebpf::NEG64      if !tbpf_version.disable_neg() => {},
                ebpf::ST_4B_REG  if tbpf_version.move_memory_instruction_classes() => store = true,
                ebpf::MOD64_IMM  if !tbpf_version.enable_pqr() => { check_imm_nonzero(&insn, insn_ptr)?; },
                ebpf::ST_8B_IMM  if tbpf_version.move_memory_instruction_classes() => store = true,
                ebpf::MOD64_REG  if !tbpf_version.enable_pqr() => {},
                ebpf::ST_8B_REG  if tbpf_version.move_memory_instruction_classes() => store = true,
                ebpf::XOR64_IMM  => {},
                ebpf::XOR64_REG  => {},
                ebpf::MOV64_IMM  => {},
                ebpf::MOV64_REG  => {},
                ...
                ebpf::ARSH64_REG => {},
                ebpf::HOR64_IMM  if tbpf_version.disable_lddw() => {},

                // BPF_PQR class
                ebpf::LMUL32_IMM if tbpf_version.enable_pqr() => {},
                ebpf::LMUL32_REG if tbpf_version.enable_pqr() => {},
                ebpf::LMUL64_IMM if tbpf_version.enable_pqr() => {},
                ebpf::LMUL64_REG if tbpf_version.enable_pqr() => {},
                ebpf::UHMUL64_IMM if tbpf_version.enable_pqr() => {},
                ebpf::UHMUL64_REG if tbpf_version.enable_pqr() => {},
                ebpf::SHMUL64_IMM if tbpf_version.enable_pqr() => {},
                ebpf::SHMUL64_REG if tbpf_version.enable_pqr() => {},
                ebpf::UDIV32_IMM if tbpf_version.enable_pqr() => { check_imm_nonzero(&insn, insn_ptr)?; },
                ebpf::UDIV32_REG if tbpf_version.enable_pqr() => {},
                ebpf::UDIV64_IMM if tbpf_version.enable_pqr() => { check_imm_nonzero(&insn, insn_ptr)?; },
                ebpf::UDIV64_REG if tbpf_version.enable_pqr() => {},
                ebpf::UREM32_IMM if tbpf_version.enable_pqr() => { check_imm_nonzero(&insn, insn_ptr)?; },
                ebpf::UREM32_REG if tbpf_version.enable_pqr() => {},
                ebpf::UREM64_IMM if tbpf_version.enable_pqr() => { check_imm_nonzero(&insn, insn_ptr)?; },
                ebpf::UREM64_REG if tbpf_version.enable_pqr() => {},
                ebpf::SDIV32_IMM if tbpf_version.enable_pqr() => { check_imm_nonzero(&insn, insn_ptr)?; },
                ebpf::SDIV32_REG if tbpf_version.enable_pqr() => {},
                ebpf::SDIV64_IMM if tbpf_version.enable_pqr() => { check_imm_nonzero(&insn, insn_ptr)?; },
                ebpf::SDIV64_REG if tbpf_version.enable_pqr() => {},
                ebpf::SREM32_IMM if tbpf_version.enable_pqr() => { check_imm_nonzero(&insn, insn_ptr)?; },
                ebpf::SREM32_REG if tbpf_version.enable_pqr() => {},
                ebpf::SREM64_IMM if tbpf_version.enable_pqr() => { check_imm_nonzero(&insn, insn_ptr)?; },
                ebpf::SREM64_REG if tbpf_version.enable_pqr() => {},
                ...
                ebpf::CALL_IMM   if tbpf_version.static_syscalls() && insn.src == 1 => {
                    let target_pc = tbpf_version.calculate_call_imm_target_pc(insn_ptr, insn.imm);
                ...
                ebpf::CALL_IMM   if tbpf_version.static_syscalls() && insn.src == 0 => {
                    syscall_registry
                        .lookup_by_key(insn.imm as u32)
                        .map(|_| ())
                        .ok_or(VerifierError::InvalidSyscall(insn.imm as u32))?;
                },
                ebpf::CALL_IMM  if !tbpf_version.static_syscalls() => {},
                ebpf::CALL_REG   => { check_callx_register(&insn, insn_ptr, tbpf_version)?; },
                ebpf::EXIT       => {},
            ...
            check_registers(&insn, store, insn_ptr, tbpf_version)?;
