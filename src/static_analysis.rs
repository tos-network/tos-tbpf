...
impl<'a> Analysis<'a> {
    ...
    pub fn from_executable<C: ContextObject>(
        ...
        let mut result = Self {
            // SAFETY: Transmuting executable reference is safe because we're only analyzing
            // the program structure, not executing it. DummyContextObject is only used as a
            // type parameter and has the same memory layout.
            executable: unsafe {
        ...
        result.split_into_basic_blocks(false, executable.get_tbpf_version());
    ...
    pub fn split_into_basic_blocks(&mut self, flatten_call_graph: bool, tbpf_version: TBPFVersion) {
        ...
        for (pc, insn) in self.instructions.iter().enumerate() {
            ...
            match insn.opc {
                ...
                ebpf::EXIT if !tbpf_version.static_syscalls() => {
                    self.cfg_nodes.entry(insn.ptr + 1).or_default();
