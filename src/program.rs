...
/// Defines a set of tbpf_version of an executable
#[derive(Debug, PartialEq, PartialOrd, Eq, Clone, Copy)]
pub enum TBPFVersion {
    /// The legacy format
    V0,
    /// SIMD-0166
    V1,
...
impl TBPFVersion {
    /// Enable SIMD-0166: TBPF dynamic stack frames
    pub fn dynamic_stack_frames(self) -> bool {
        self >= TBPFVersion::V1
    }

    /// Enable SIMD-0174: TBPF arithmetics improvements
    ...
    /// Enable SIMD-0173: TBPF instruction encoding improvements
    ...
    /// Enable SIMD-0178: TBPF Static Syscalls
    pub fn static_syscalls(self) -> bool {
        self >= TBPFVersion::V3
    }
    /// Enable SIMD-0179: TBPF stricter verification constraints
    pub fn enable_stricter_verification(self) -> bool {
        self >= TBPFVersion::V3
    }
    /// Enable SIMD-0189: TBPF stricter ELF headers
...
impl<T: Copy + PartialEq> FunctionRegistry<T> {
    ...
    /// Used for transitioning from TBPFv0 to TBPFv3
...
impl<C: ContextObject> BuiltinProgram<C> {
    ...
    /// Get the function registry depending on the TBPF version
...
impl<C: ContextObject> std::fmt::Debug for BuiltinProgram<C> {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> Result<(), std::fmt::Error> {
        // SAFETY: Transmuting to usize for Debug is safe as we're only using it for formatting.
        // The memory layout is compatible and we don't access the transmuted value beyond Debug.
        unsafe {
            writeln!(
                f,
                "registry: {:?}",
