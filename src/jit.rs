...
// Copyright 2020 Tos Maintainers <info@tos.network>
//
...
impl JitProgram {
    /// Creates a new JIT program with allocated memory for code generation.
    ///
    /// # Safety Considerations
    ///
    /// This function allocates executable memory and creates mutable slices from raw pointers.
    /// All size calculations must be carefully validated to prevent:
    ...
    fn new(pc: usize, code_size: usize) -> Result<Self, EbpfError> {
        ...
        // Pre-condition: Check for integer overflow in size calculations
        let pc_loc_table_size = pc
            .checked_mul(std::mem::size_of::<u32>())
            .ok_or(EbpfError::InvalidMemoryRegion(0))?;
        let pc_loc_table_size = round_to_page_size(pc_loc_table_size, page_size);

        let over_allocated_code_size = round_to_page_size(code_size, page_size);

        // Check total allocation size
        let total_size = pc_loc_table_size
            .checked_add(over_allocated_code_size)
            .ok_or(EbpfError::InvalidMemoryRegion(0))?;

        // Sanity check: Limit maximum JIT allocation
        const MAX_JIT_ALLOCATION: usize = 256 << 20; // 256 MB
        if total_size > MAX_JIT_ALLOCATION {
            return Err(EbpfError::InvalidMemoryRegion(0));
        }

        // SAFETY: allocate_pages returns a valid, non-null pointer to executable memory of size total_size.
        // All size calculations have been validated for overflow and bounded by MAX_JIT_ALLOCATION.
        // The allocated memory is page-aligned and will be properly freed in Drop.
        unsafe {
            let raw = allocate_pages(total_size)?;

            // Post-condition: Verify allocation succeeded correctly
            debug_assert!(!raw.is_null());
            debug_assert!((raw as usize).is_multiple_of(page_size));
    ...
    fn seal(&mut self, text_section_usage: usize) -> Result<(), EbpfError> {
        ...
        // SAFETY: All pointer arithmetic and memory operations are within bounds of the originally allocated memory.
        // raw points to the start of the allocated region, and all offsets (pc_loc_table_size, text_section_usage, code_size)
        // are validated to be within the allocated size. Memory protection changes are applied to page-aligned regions.
        unsafe {
    ...
    pub(crate) fn invoke<C: ContextObject>(
        ...
        // SAFETY: The JIT code is properly compiled and sealed. The inline assembly follows the System V ABI,
        // preserving required registers (RBP, RBX) and maintaining stack alignment. The runtime_environment pointer
        // is derived from a valid vm reference, and all register values are properly saved/restored across the call.
        unsafe {
...
impl Drop for JitProgram {
    fn drop(&mut self) {
        ...
        if pc_loc_table_size + code_size > 0 {
            // SAFETY: The pointer is from the originally allocated memory region, and the size matches
            // the original allocation (pc_loc_table_size + code_size). This is the corresponding free
            // operation for the allocation in JitProgram::new().
            unsafe {
                let _ = free_pages(
...
impl<'a, C: ContextObject> JitCompiler<'a, C> {
    ...
    pub fn new(executable: &'a Executable<C>) -> Result<Self, EbpfError> {
        ...
        let mut pc = 0;
        if !executable.get_tbpf_version().disable_lddw() {
    ...
    pub fn compile(mut self) -> Result<JitProgram, EbpfError> {
        ...
        while self.pc * ebpf::INSN_SIZE < self.program.len() {
            ...
            if self.executable.get_tbpf_version().enable_stricter_verification() &&
            ...
            match insn.opc {
                ebpf::LD_DW_IMM if !self.executable.get_tbpf_version().disable_lddw() => {
                    self.emit_validate_and_profile_instruction_count(Some(self.pc + 2));
                    self.pc += 1;
                    // SAFETY: ANCHOR_CALL_UNSUPPORTED_INSTRUCTION has been set during emit_subroutines() before compilation.
                    // Both pointers are within text_section bounds, making offset_from well-defined and producing a valid i32 offset.
                ...
                // BPF_LDX class
                ebpf::LD_B_REG  if !self.executable.get_tbpf_version().move_memory_instruction_classes() => {
                    self.emit_address_translation(Some(dst), Value::RegisterPlusConstant64(src, insn.off as i64, true), 1, None);
                },
                ebpf::LD_H_REG  if !self.executable.get_tbpf_version().move_memory_instruction_classes() => {
                    self.emit_address_translation(Some(dst), Value::RegisterPlusConstant64(src, insn.off as i64, true), 2, None);
                },
                ebpf::LD_W_REG  if !self.executable.get_tbpf_version().move_memory_instruction_classes() => {
                    self.emit_address_translation(Some(dst), Value::RegisterPlusConstant64(src, insn.off as i64, true), 4, None);
                },
                ebpf::LD_DW_REG if !self.executable.get_tbpf_version().move_memory_instruction_classes() => {
                    self.emit_address_translation(Some(dst), Value::RegisterPlusConstant64(src, insn.off as i64, true), 8, None);
                },

                // BPF_ST class
                ebpf::ST_B_IMM  if !self.executable.get_tbpf_version().move_memory_instruction_classes() => {
                ...
                ebpf::ST_H_IMM  if !self.executable.get_tbpf_version().move_memory_instruction_classes() => {
                ...
                ebpf::ST_W_IMM  if !self.executable.get_tbpf_version().move_memory_instruction_classes() => {
                ...
                ebpf::ST_DW_IMM if !self.executable.get_tbpf_version().move_memory_instruction_classes() => {
                ...
                ebpf::ADD32_IMM  => {
                    ...
                    if !self.executable.get_tbpf_version().explicit_sign_extension_of_results() {
                ...
                ebpf::SUB32_IMM  => {
                    if self.executable.get_tbpf_version().swap_sub_reg_imm_operands() {
                ...
                ebpf::MUL32_IMM if !self.executable.get_tbpf_version().enable_pqr() => {
                    if self.should_sanitize_constant(insn.imm) {
                ...
                ebpf::DIV32_IMM | ebpf::MOD32_IMM if !self.executable.get_tbpf_version().enable_pqr() =>
                ...
                ebpf::LD_1B_REG  if self.executable.get_tbpf_version().move_memory_instruction_classes() => {
                    self.emit_address_translation(Some(dst), Value::RegisterPlusConstant64(src, insn.off as i64, true), 1, None);
                },
                ebpf::MUL32_REG if !self.executable.get_tbpf_version().enable_pqr() => {
                    self.emit_ins(X86Instruction::alu_escaped(OperandSize::S32, 1, 0xaf, dst, src, None));
                    if !self.executable.get_tbpf_version().explicit_sign_extension_of_results() {
                        self.emit_ins(X86Instruction::alu(OperandSize::S64, 0x63, dst, dst, None)); // sign extend i32 to i64
                    }
                },
                ebpf::DIV32_REG | ebpf::MOD32_REG if !self.executable.get_tbpf_version().enable_pqr() =>
                    self.emit_product_quotient_remainder(
                        OperandSize::S32,
                        (insn.opc & ebpf::BPF_ALU_OP_MASK) == ebpf::BPF_MOD,
                        (insn.opc & ebpf::BPF_ALU_OP_MASK) != ebpf::BPF_MUL,
                ...
                ebpf::LD_2B_REG  if self.executable.get_tbpf_version().move_memory_instruction_classes() => {
                    self.emit_address_translation(Some(dst), Value::RegisterPlusConstant64(src, insn.off as i64, true), 2, None);
                },
                ...
                ebpf::NEG32      if !self.executable.get_tbpf_version().disable_neg() => self.emit_ins(X86Instruction::alu_immediate(OperandSize::S32, 0xf7, 3, dst, 0, None)),
                ebpf::LD_4B_REG  if self.executable.get_tbpf_version().move_memory_instruction_classes() => {
                    self.emit_address_translation(Some(dst), Value::RegisterPlusConstant64(src, insn.off as i64, true), 4, None);
                },
                ebpf::LD_8B_REG  if self.executable.get_tbpf_version().move_memory_instruction_classes() => {
                    self.emit_address_translation(Some(dst), Value::RegisterPlusConstant64(src, insn.off as i64, true), 8, None);
                },
                ...
                ebpf::LE if !self.executable.get_tbpf_version().disable_le() => {
                    match insn.imm {
                        16 => {
                ...
                ebpf::MUL64_IMM if !self.executable.get_tbpf_version().enable_pqr() => {
                    if self.should_sanitize_constant(insn.imm) {
                        self.emit_sanitized_load_immediate(REGISTER_SCRATCH, insn.imm);
                    } else {
                        self.emit_ins(X86Instruction::load_immediate(REGISTER_SCRATCH, insn.imm));
                ...
                ebpf::DIV64_IMM | ebpf::MOD64_IMM if !self.executable.get_tbpf_version().enable_pqr() =>
                    self.emit_product_quotient_remainder(
                        OperandSize::S64,
                        (insn.opc & ebpf::BPF_ALU_OP_MASK) == ebpf::BPF_MOD,
                        (insn.opc & ebpf::BPF_ALU_OP_MASK) != ebpf::BPF_MUL,
                ...
                ebpf::ST_1B_IMM  if self.executable.get_tbpf_version().move_memory_instruction_classes() => {
                    self.emit_address_translation(None, Value::RegisterPlusConstant64(dst, insn.off as i64, true), 1, Some(Value::Constant64(insn.imm, true)));
                },
                ebpf::ST_2B_IMM  if self.executable.get_tbpf_version().move_memory_instruction_classes() => {
                    self.emit_address_translation(None, Value::RegisterPlusConstant64(dst, insn.off as i64, true), 2, Some(Value::Constant64(insn.imm, true)));
                },
                ebpf::MUL64_REG if !self.executable.get_tbpf_version().enable_pqr() => {
                    self.emit_ins(X86Instruction::alu_escaped(OperandSize::S64, 1, 0xaf, dst, src, None));
                },
                ebpf::DIV64_REG | ebpf::MOD64_REG if !self.executable.get_tbpf_version().enable_pqr() =>
                    self.emit_product_quotient_remainder(
                        OperandSize::S64,
                        (insn.opc & ebpf::BPF_ALU_OP_MASK) == ebpf::BPF_MOD,
                        (insn.opc & ebpf::BPF_ALU_OP_MASK) != ebpf::BPF_MUL,
                ...
                ebpf::ST_1B_REG  if self.executable.get_tbpf_version().move_memory_instruction_classes() => {
                    self.emit_address_translation(None, Value::RegisterPlusConstant64(dst, insn.off as i64, true), 1, Some(Value::Register(src)));
                },
                ebpf::ST_2B_REG  if self.executable.get_tbpf_version().move_memory_instruction_classes() => {
                    self.emit_address_translation(None, Value::RegisterPlusConstant64(dst, insn.off as i64, true), 2, Some(Value::Register(src)));
                },
                ...
                ebpf::ST_4B_IMM  if self.executable.get_tbpf_version().move_memory_instruction_classes() => {
                    self.emit_address_translation(None, Value::RegisterPlusConstant64(dst, insn.off as i64, true), 4, Some(Value::Constant64(insn.imm, true)));
                },
                ebpf::NEG64      if !self.executable.get_tbpf_version().disable_neg() => self.emit_ins(X86Instruction::alu_immediate(OperandSize::S64, 0xf7, 3, dst, 0, None)),
                ebpf::ST_4B_REG  if self.executable.get_tbpf_version().move_memory_instruction_classes() => {
                    self.emit_address_translation(None, Value::RegisterPlusConstant64(dst, insn.off as i64, true), 4, Some(Value::Register(src)));
                },
                ebpf::ST_8B_IMM  if self.executable.get_tbpf_version().move_memory_instruction_classes() => {
                    self.emit_address_translation(None, Value::RegisterPlusConstant64(dst, insn.off as i64, true), 8, Some(Value::Constant64(insn.imm, true)));
                },
                ebpf::ST_8B_REG  if self.executable.get_tbpf_version().move_memory_instruction_classes() => {
                    self.emit_address_translation(None, Value::RegisterPlusConstant64(dst, insn.off as i64, true), 8, Some(Value::Register(src)));
                },
                ...
                ebpf::HOR64_IMM if self.executable.get_tbpf_version().disable_lddw() => {
                ...
                if self.executable.get_tbpf_version().enable_pqr() => {
                ...
                ebpf::CALL_IMM => {
                    ...
                    if self.executable.get_tbpf_version().static_syscalls() && insn.src == 1 {
                        // BPF to BPF call
                ...
                ebpf::CALL_REG  => {
                    let target_pc = if self.executable.get_tbpf_version().callx_uses_src_reg() {
                        src
                    } else {
    ...
    pub(crate) fn emit<T>(&mut self, data: T) {
        // SAFETY: offset_in_text_section is always maintained within text_section bounds.
        // The compiler checks ensure sufficient space before emitting instructions (see MAX_MACHINE_CODE_LENGTH_PER_INSTRUCTION checks).
        // write_unaligned is used to handle potentially unaligned writes safely.
        unsafe {
    ...
    fn emit_product_quotient_remainder(
        ...
        if let OperandSize::S32 = size {
            if signed && !self.executable.get_tbpf_version().explicit_sign_extension_of_results() {
                self.emit_ins(X86Instruction::alu(OperandSize::S64, 0x63, dst, dst, None)); // sign extend i32 to i64
            }
        }
    }
    ...
    fn emit_set_exception_kind(&mut self, err: EbpfError) {
        // SAFETY: EbpfError is repr(C) with a u64 discriminant as its first field.
        // Reading the first 8 bytes as u64 through addr_of! is safe and extracts the error kind discriminant.
    ...
    fn emit_subroutines(&mut self) {
        ...
        if !self.executable.get_tbpf_version().dynamic_stack_frames() {
    ...
    /// Sets an anchor point in the generated code for later reference.
    ///
    /// # Safety Invariants
    /// - `anchor` must be < ANCHOR_COUNT
    /// - `offset_in_text_section` must be <= text_section.len()
    fn set_anchor(&mut self, anchor: usize) {
        debug_assert!(anchor < ANCHOR_COUNT, "Anchor {} out of bounds", anchor);
        debug_assert!(
            self.offset_in_text_section <= self.result.text_section.len(),
            "Offset {} exceeds text section length {}",
            self.offset_in_text_section,
            self.result.text_section.len()
        );

        // SAFETY: offset_in_text_section is verified by debug_assert to be within text_section bounds.
        // The resulting pointer is valid for the lifetime of the JitProgram and will be used for relative offset calculations.
        self.anchors[anchor] = unsafe { self.result.text_section.as_ptr().add(self.offset_in_text_section) };

        debug_assert!(!self.anchors[anchor].is_null());
    }

    /// Calculates the relative offset from the current position to an anchor.
    ///
    /// # Arguments
    /// - `instruction_length`: 5 for unconditional jump/call, 6 for conditional jump
    ///
    /// # Safety Invariants
    /// - Both pointers must be within the text_section
    /// - The anchor must have been set previously
    fn relative_to_anchor(&self, anchor: usize, instruction_length: usize) -> i32 {
        debug_assert!(anchor < ANCHOR_COUNT);
        debug_assert!(!self.anchors[anchor].is_null(), "Anchor {} not set", anchor);
        debug_assert!(
            self.offset_in_text_section + instruction_length <= self.result.text_section.len()
        );

        // SAFETY: offset_in_text_section + instruction_length is verified to be within text_section bounds.
        // The pointer arithmetic produces a valid pointer within the allocated text_section.
        ...
        // Verify both pointers are within text_section
        let text_start = self.result.text_section.as_ptr();
        // SAFETY: text_section.len() is the valid length of the slice, so adding it to the start pointer
        // produces the one-past-the-end pointer, which is valid for comparison purposes.
        let text_end = unsafe { text_start.add(self.result.text_section.len()) };
        debug_assert!(instruction_end >= text_start && instruction_end <= text_end);
        debug_assert!(destination >= text_start && destination <= text_end);

        debug_assert!(!destination.is_null());
        // SAFETY: Both destination and instruction_end are verified to be within text_section bounds.
        // offset_from is well-defined between pointers to the same allocated object (text_section).
    ...
    fn relative_to_target_pc(&mut self, target_pc: usize, instruction_length: usize) -> i32 {
        // SAFETY: offset_in_text_section + instruction_length is bounded by MAX_MACHINE_CODE_LENGTH_PER_INSTRUCTION checks.
        // The pointer arithmetic stays within the allocated text_section.
        let instruction_end = unsafe { self.result.text_section.as_ptr().add(self.offset_in_text_section).add(instruction_length) };
        let destination = if self.result.pc_section[target_pc] != 0 {
            ...
            // SAFETY: instruction_end points to the end of a jump instruction, which includes a 4-byte offset field.
            // Subtracting 4 produces a pointer to the offset field within the instruction, which is within text_section bounds.
            self.text_section_jumps.push(Jump { location: unsafe { instruction_end.sub(4) }, target_pc });
            return 0;
        };
        debug_assert!(!destination.is_null());
        // SAFETY: Both destination and instruction_end point within text_section.
        // offset_from is well-defined between pointers to the same allocated object.
        (unsafe { destination.offset_from(instruction_end) } as i32) // Relative jump
    }
    ...
    fn resolve_jumps(&mut self) {
        ...
        for jump in &self.text_section_jumps {
            ...
            // SAFETY: Both destination and jump.location point within text_section (jump.location was created via instruction_end.sub(4)).
            // offset_from is well-defined, and the result fits in i32 for relative jumps in the compiled code.
            let offset_value =
                unsafe { destination.offset_from(jump.location) } as i32 // Relative jump
                - mem::size_of::<i32>() as i32; // Jump from end of instruction
            // SAFETY: jump.location points to the 4-byte offset field within a jump instruction in text_section.
            // write_unaligned safely writes the i32 offset value to this location.
