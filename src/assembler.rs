...
fn make_instruction_map(tbpf_version: TBPFVersion) -> HashMap<String, (InstructionType, u8)> {
...

    {
        ...
        // Memory
        if tbpf_version.move_memory_instruction_classes() {
...
///
/// ```
/// use tos_tbpf::{assembler::assemble, program::BuiltinProgram, vm::Config};
...
pub fn assemble<C: ContextObject>(
    ...
    let tbpf_version = *loader.get_config().enabled_tbpf_versions.end();

    let statements = parse(src)?;
    let instruction_map = make_instruction_map(tbpf_version);
    let mut insn_ptr = 0;
                    ...
                    Some(&(inst_type, opc)) => {
                        let mut insn = match (inst_type, operands.as_slice()) {
                            ...
                            (AluBinary, [Register(10), Integer(imm)]) if opc == ebpf::ADD64_IMM => {
                                if tbpf_version.dynamic_stack_frames() {
                            ...
                            (CallImm, [Integer(imm)]) => {
                                ...
                                function_registry
                                    ...
                                    .map_err(|_| format!("Label hash collision {name}"))?;
                                let instr_imm = if tbpf_version.static_syscalls() {
                                    *imm
                                } else {
                                    target_pc
                                };
                                insn(opc, 0, tbpf_version.static_syscalls() as i64, 0, instr_imm)
                            }
                            (CallImm, [Label(label)]) => {
                                let label: &str = label;
                                let mut target_pc = *labels
                                    ...
                                    .ok_or_else(|| format!("Label not found {label}"))?
                                    as i64;
                                if tbpf_version.static_syscalls() {
                                    target_pc = target_pc - insn_ptr as i64 - 1;
                                }
                                insn(opc, 0, tbpf_version.static_syscalls() as i64, 0, target_pc)
                            }
                            ...
                            (CallReg, [Register(dst)]) => {
                                if tbpf_version.callx_uses_src_reg() {
                                    insn(opc, 0, *dst, 0, 0)
                                } else {
                                    insn(opc, 0, 0, 0, *dst)
                                }
    ...
    let program = instructions
        ...
        .collect::<Vec<_>>();
    Executable::<C>::from_text_bytes(&program, loader, tbpf_version, function_registry)
