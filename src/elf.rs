...
use crate::{
    ...
    elf_parser::{
        consts::{
            ELFCLASS64, ELFDATA2LSB, ELFOSABI_NONE, EM_BPF, EM_TBPF, ET_DYN, R_X86_64_32,
...
pub enum ElfError {
    ...
    ValueOutOfBounds,
    /// Detected tbpf_version required by the executable which are not enabled
    #[error("Detected tbpf_version required by the executable which are not enabled")]
    UnsupportedTBPFVersion,
...
impl<C: ContextObject> Executable<C> {
    ...
    /// Get the executable tbpf_version
    pub fn get_tbpf_version(&self) -> TBPFVersion {
        self.tbpf_version
    }
    ...
    pub fn new_from_text_bytes(
        ...
        Ok(Self {
            ...
            text_section_vaddr: if tbpf_version.enable_lower_bytecode_vaddr() {
    ...
    pub fn load(bytes: &[u8], loader: Arc<BuiltinProgram<C>>) -> Result<Self, ElfError> {
        ...
        let tbpf_version = match e_flags {
            0 => TBPFVersion::V0,
            1 => TBPFVersion::V1,
            2 => TBPFVersion::V2,
            3 => TBPFVersion::V3,
            4 => TBPFVersion::V4,
            _ => TBPFVersion::Reserved,
        };
        if !config.enabled_tbpf_versions.contains(&tbpf_version) {
            return Err(ElfError::UnsupportedTBPFVersion);
        }

        let mut executable = if tbpf_version.enable_stricter_elf_headers() {
            Self::load_with_strict_parser(bytes, loader)?
        } else {
            Self::load_with_lenient_parser(bytes, loader)?
        };
        executable.tbpf_version = tbpf_version;
        Ok(executable)
    }
    ...
    pub fn load_with_strict_parser(
        ...
        Ok(Self {
            ...
            tbpf_version: TBPFVersion::Reserved, // Is set in Self::load()
            ro_section,
            text_section_vaddr,
            text_section_range,
            entry_pc,
    ...
    pub fn validate(elf: &Elf64, elf_bytes: &[u8]) -> Result<(), ElfError> {
        ...
        if header.e_machine != EM_BPF && header.e_machine != EM_TBPF {
