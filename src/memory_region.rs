...
impl CommonMemoryMapping<'_> {
    fn generate_access_violation(
        ...
        let stack_frame = (vm_addr as i64)
            ...
            .unwrap_or(0);
        if !self.tbpf_version.dynamic_stack_frames()
...
impl<'a> UnalignedMemoryMapping<'a> {
    ...
    pub fn new_with_access_violation_handler(
        ...
        debug_assert!(
            tbpf_version <= TBPFVersion::V3,
            "TBPFv4 and later versions do not support unaligned memory"
        );
        regions.sort();
    ...
    /// # Safety Invariants
    ///
    /// - The cache reference is guaranteed unique (UnalignedMemoryMapping is !Sync)
    /// - All array accesses are bounds-checked in debug builds
    /// - The Eytzinger tree construction ensures all indices are valid
    #[allow(clippy::arithmetic_side_effects)]
    pub fn find_region(&self, vm_addr: u64) -> Option<(usize, &MemoryRegion)> {
        // SAFETY: UnalignedMemoryMapping is !Sync, ensuring this is the only mutable
        // reference to the cache. The cache is only mutated in this function.
        let cache = unsafe { &mut *self.cache.get() };

        if let Some(index) = cache.find(vm_addr) {
            // INVARIANT: Cached indices are validated before insertion (see below)
            debug_assert!(
                index < self.common.regions.len(),
                "Cached index {} exceeds regions length {}",
                index,
                self.common.regions.len()
            );

            // SAFETY: Index was validated when cached
            Some((index, unsafe { self.common.regions.get_unchecked(index) }))
        } else {
            // Eytzinger binary search through the region addresses
            let mut index = 1;
            while index <= self.region_addresses.len() {
                // INVARIANT: Loop starts at 1 and continues while index <= len
                // Therefore: 1 <= index <= len, which means index-1 < len
                debug_assert!(
                    index >= 1 && index <= self.region_addresses.len(),
                    "Eytzinger index {} out of valid range [1, {}]",
                    index,
                    self.region_addresses.len()
                );

                // SAFETY: index - 1 < region_addresses.len() by loop invariant
                index = (index << 1)
                    + unsafe { *self.region_addresses.get_unchecked(index - 1) <= vm_addr }
                        as usize;
            }

            // Convert Eytzinger position to array index
            ...
            // INVARIANT: By Eytzinger tree construction, if index > 0 here,
            // then index <= region_index_lookup.len()
            debug_assert!(
                index > 0 && index <= self.region_index_lookup.len(),
                "Region lookup index {} out of valid range (0, {}]",
                index,
                self.region_index_lookup.len()
            );

            // SAFETY: Validated by assertion above and Eytzinger tree invariants
            index = unsafe { *self.region_index_lookup.get_unchecked(index - 1) };

            // Final validation before returning
            debug_assert!(
                index < self.common.regions.len(),
                "Final region index {} exceeds length {}",
                index,
                self.common.regions.len()
            );

            // SAFETY: Index validated immediately above
...
impl<'a> MemoryMapping<'a> {
    ...
    pub fn new_with_access_violation_handler(
        ...
        if tbpf_version == TBPFVersion::V4 || config.aligned_memory_mapping {
    ...
    pub fn load<T: Pod + Into<u64>>(&mut self, vm_addr: u64) -> ProgramResult {
        ...
        match self.map_with_access_violation_handler(AccessType::Load, vm_addr, len) {
            ProgramResult::Ok(host_addr) => {
                // SAFETY: host_addr is validated by map_with_access_violation_handler to be within bounds
...
mod test {
    ...
    fn test_map_empty() {
        for aligned_memory_mapping in [false, true] {
            ...
            let m = MemoryMapping::new(vec![], &config, TBPFVersion::V3).unwrap();
            assert_error!(
