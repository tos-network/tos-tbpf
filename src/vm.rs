...
use crate::{
    ...
    program::{BuiltinFunction, BuiltinProgram, FunctionRegistry, TBPFVersion},
...
pub struct Config {
    ...
    /// Allowed [TBPFVersion]s
    pub enabled_tbpf_versions: std::ops::RangeInclusive<TBPFVersion>,
}
...
impl<C: ContextObject> Executable<C> {
    ...
    pub fn from_text_bytes(
        ...
        Executable::new_from_text_bytes(text_bytes, loader, tbpf_version, function_registry)
...
/// let mut executable = Executable::<TestContextObject>::from_text_bytes(prog, loader.clone(), TBPFVersion::V4, function_registry).unwrap();
/// executable.verify::<RequisiteVerifier>().unwrap();
/// let mut context_object = TestContextObject::new(2);
/// let tbpf_version = executable.get_tbpf_version();
///
...
/// ];
///
/// let memory_mapping = MemoryMapping::new(regions, executable.get_config(), tbpf_version).unwrap();
///
/// let mut vm = EbpfVm::new(loader, tbpf_version, &mut context_object, memory_mapping, stack_len);
///
...
impl<'a, C: ContextObject> EbpfVm<'a, C> {
    ...
    pub fn new(
        ...
        registers[ebpf::FRAME_PTR_REG] =
            ebpf::MM_STACK_START.saturating_add(if tbpf_version.dynamic_stack_frames() {
    ...
    pub fn invoke_function(&mut self, function: BuiltinFunction<C>) {
        // SAFETY: Pointer manipulation for runtime environment key is safe because:
        // 1. The key is used for pointer tagging/validation
        // 2. The offset is within valid memory bounds
        // 3. The cast back to Self is valid as we're working with the same object
        function(
            unsafe {
