...
fn test_memory_instructions() {
    for tbpf_version in [TBPFVersion::V0, TBPFVersion::V4] {
        let config = Config {
...
fn test_hor64() {
    let config = Config {
        enabled_tbpf_versions: TBPFVersion::V2..=TBPFVersion::V2,
        ..Config::default()
    };
    test_interpreter_and_jit_asm!(
        "
...
fn test_err_dynamic_stack_out_of_bound() {
    let config = Config {
        enabled_tbpf_versions: TBPFVersion::V0..=TBPFVersion::V4,
        max_call_depth: 3,
        ..Config::default()
    };
...
fn test_entrypoint_exit() {
    ...
    // frames.
    for highest_tbpf_version in [TBPFVersion::V0, TBPFVersion::V4] {
        let config = Config {
            enabled_tbpf_versions: TBPFVersion::V0..=highest_tbpf_version,
            ..Config::default()
        };
...
#[test]
fn test_relative_call_tbpfv0() {
    let config = Config {
        enabled_tbpf_versions: TBPFVersion::V0..=TBPFVersion::V0,
        ..Config::default()
    };
    test_interpreter_and_jit_elf!(
        "tests/elfs/relative_call_tbpfv0.so",
        config,
        [1],
        (),
...
#[test]
fn test_relative_call_tbpfv3() {
    let config = Config {
        enabled_tbpf_versions: TBPFVersion::V3..=TBPFVersion::V4,
        ..Config::default()
    };
    test_interpreter_and_jit_elf!(
...
fn test_err_callx_unregistered() {
    ...
    let versions = [TBPFVersion::V0, TBPFVersion::V4];
    let expected_errors = [
    ...
    for (version, error) in versions.iter().zip(expected_errors) {
        let config = Config {
            enabled_tbpf_versions: *version..=*version,
            ..Config::default()
        };
...
declare_builtin_function!(
    ...
    fn rust(
        ...
        if depth > 0 {
            ...
            if version == 0 {
                config.enabled_tbpf_versions = TBPFVersion::V0..=TBPFVersion::V0;
            } else {
                config.enabled_tbpf_versions = TBPFVersion::V3..=TBPFVersion::V4;
            };
...
fn test_nested_vm_syscall() {
    ...
    let mut memory_mapping = MemoryMapping::new(vec![], &config, TBPFVersion::V4).unwrap();
...
fn test_syscall_reloc_64_32() {
    ...
    test_interpreter_and_jit_elf!(
        "tests/elfs/syscall_reloc_64_32_tbpfv0.so",
        config,
        [],
        (
            "log" => syscalls::SyscallString::vm,
...
fn test_reloc_64_64_tbpfv0() {
    ...
    test_interpreter_and_jit_elf!(
        "tests/elfs/reloc_64_64_tbpfv0.so",
        [],
        (),
        TestContextObject::new(2),
...
#[test]
fn test_reloc_64_relative_tbpfv0() {
    ...
    test_interpreter_and_jit_elf!(
        "tests/elfs/reloc_64_relative_tbpfv0.so",
        config,
        [],
        (),
        TestContextObject::new(2),
...
#[test]
fn test_reloc_64_relative_data_tbpfv0() {
    // we used to
    ...
    // compatibility when dealing with non-tbpfv3 files. See also Elf::relocate().
    //
    ...
    test_interpreter_and_jit_elf!(
        "tests/elfs/reloc_64_relative_data_tbpfv0.so",
        config,
        [],
        (),
        TestContextObject::new(3),
...
#[test]
fn test_load_elf_rodata_tbpfv0() {
    let config = Config {
        enabled_tbpf_versions: TBPFVersion::V0..=TBPFVersion::V0,
    ...
    test_interpreter_and_jit_elf!(
        "tests/elfs/rodata_section_tbpfv0.so",
        config,
        [],
        (),
        TestContextObject::new(3),
...
#[test]
fn test_struct_func_pointer_tbpfv0() {
    ...
    test_interpreter_and_jit_elf!(
        "tests/elfs/struct_func_pointer_tbpfv0.so",
        config,
        [],
        (),
        TestContextObject::new(2),
...
fn execute_generated_program(prog: &[u8]) -> bool {
    ...
    {
        let analysis = tos_tbpf::static_analysis::Analysis::from_executable(&executable).unwrap();
