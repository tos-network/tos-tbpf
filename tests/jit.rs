...
fn create_mockup_executable(config: Config, program: &[u8]) -> Executable<TestContextObject> {
    let tbpf_version = *config.enabled_tbpf_versions.end();
...
fn test_code_length_estimate() {
    ...
    for tbpf_version in [TBPFVersion::V0, TBPFVersion::V3] {
        ...
        empty_program_machine_code_length_per_version[tbpf_version as usize] =
    ...
    for tbpf_version in [TBPFVersion::V0, TBPFVersion::V3] {
        ...
        for mut opcode in 0x00..=0xFF {
            let (registers, immediate) = match opcode {
                0x85 if !tbpf_version.static_syscalls() => (0x00, Some(8)),
                0x85 if tbpf_version.static_syscalls() => (0x00, None),
                0x8D => (0x88, Some(0)),
                0x95 if tbpf_version.static_syscalls() => (0x00, Some(0x91020CDD)),
                0xE5 if !tbpf_version.static_syscalls() => {
                    // Put external function calls on a separate loop iteration
                    opcode = 0x85;
                    (0x00, Some(0x91020CDD))
                }
                0xF5 if !tbpf_version.static_syscalls() => {
