...
fn test_verifier_err_invalid_reg_dst() {
    // r11 is disabled when tbpf_version.dynamic_stack_frames()=false, and only sub and add are
    // allowed when tbpf_version.dynamic_stack_frames()=true
    for highest_tbpf_version in [TBPFVersion::V0, TBPFVersion::V4] {
        let executable = assemble::<TestContextObject>(
            "
            add64 r10, 0
...
fn test_sdiv_disabled() {
    ...
    for (opc, instruction) in instructions {
        for highest_tbpf_version in [TBPFVersion::V1, TBPFVersion::V2] {
            ...
            .unwrap();
            let result = executable.verify::<RequisiteVerifier>();
            if highest_tbpf_version == TBPFVersion::V2 {
                assert!(result.is_ok());
            } else {
