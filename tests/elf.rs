...
#[test]
fn test_validate() {
    let elf_bytes = std::fs::read("tests/elfs/relative_call_tbpfv0.so").unwrap();
...
#[test]
fn test_load() {
    let mut file = File::open("tests/elfs/relative_call_tbpfv0.so").expect("file open failed");
...
fn test_load_unaligned() {
    let mut elf_bytes =
        std::fs::read("tests/elfs/relative_call_tbpfv0.so").expect("failed to read elf file");
...
fn test_owned_ro_region_no_initial_gap() {
    ...
    let memory_mapping = MemoryMapping::new(vec![ro_region], &config, TBPFVersion::V0).unwrap();
...
fn test_writable_data_section() {
    let elf_bytes =
        std::fs::read("tests/elfs/data_section_tbpfv0.so").expect("failed to read elf file");
    ElfExecutable::load(&elf_bytes, loader()).expect("validation failed");
}
...
fn test_bss_section() {
    let elf_bytes =
        std::fs::read("tests/elfs/bss_section_tbpfv0.so").expect("failed to read elf file");
    ElfExecutable::load(&elf_bytes, loader()).expect("validation failed");
}
...
fn test_err_unresolved_syscall_reloc_64_32() {
    ...
    let elf_bytes =
        std::fs::read("tests/elfs/syscall_reloc_64_32_tbpfv0.so").expect("failed to read elf file");
