...
use tos_tbpf::{ebpf, memory_region::MemoryRegion, program::TBPFVersion, vm::Config};
use tos_tbpf::{elf::Executable, program::BuiltinProgram, verifier::RequisiteVerifier};

#[bench]
fn bench_init_interpreter_start(bencher: &mut Bencher) {
    let mut file = File::open("tests/elfs/rodata_section_tbpfv0.so").unwrap();
    let mut elf = Vec::new();
...
fn bench_jit_vs_interpreter(
    ...
    let mut executable = tos_tbpf::assembler::assemble::<TestContextObject>(
        assembly,
