...
pub fn create_memory_mapping<'a, C: ContextObject>(
    ...
    let regions: Vec<MemoryRegion> = vec![
        ...
        MemoryRegion::new_writable_gapped(
            stack.as_slice_mut(),
            ebpf::MM_STACK_START,
            if !tbpf_version.dynamic_stack_frames() && config.enable_stack_frame_gaps {
    ...
    Ok(
        if let Some(access_violation_handler) = access_violation_handler {
            ...
            MemoryMapping::new(regions, config, tbpf_version)?
        },
    )
}
...
macro_rules! create_vm {
    ($vm_name:ident, $verified_executable:expr, $context_object:expr, $stack:ident, $heap:ident, $additional_regions:expr, $access_violation_handler:expr) => {
        let mut $stack = tos_tbpf::aligned_memory::AlignedMemory::zero_filled(
            $verified_executable.get_config().stack_size(),
        );
        let mut $heap = tos_tbpf::aligned_memory::AlignedMemory::with_capacity(0);
        ...
        .unwrap();
        let mut $vm_name = tos_tbpf::vm::EbpfVm::new(
            $verified_executable.get_loader().clone(),
            $verified_executable.get_tbpf_version(),
            $context_object,
            memory_mapping,
            stack_len,
        );
        $vm_name.registers[1] = tos_tbpf::ebpf::MM_INPUT_START;
    };
}
...
macro_rules! test_interpreter_and_jit {
    ...
    ($executable:expr, $mem:tt, $context_object:expr, $expected_result:expr $(,)?) => {
        ...
        if !matches!(expected_result, ProgramResult::Err(tos_tbpf::error::EbpfError::ExceededMaxInstructions)) {
...
macro_rules! test_syscall_asm {
    ...
    ($source:expr, $mem:expr, ($($syscall_name:expr => $syscall_function:expr),*$(,)?), $context_object:expr, $expected_result:expr $(,)?) => {
        ...
        for tbpf_version in [TBPFVersion::V0, TBPFVersion::V3] {
            config.enabled_tbpf_versions = tbpf_version..=tbpf_version;
